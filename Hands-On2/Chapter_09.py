...
def plot_clusters(kmeans, X, name=None):
    ...
    If a name is specified, then the plot is saved, and not shown on
    the screen
...
def test_model_maker(n_clusters):
    """A sample method that creates the actual clusterer for make_silhouette()

    The signature is important, it must consume a single n_clusters
    argument which identifies how many clusters to generate. This must
    be the only argument. All other arguments should have default
    values, since they will not be provided by make_silhouette().

    Call with:
    plot_silhouette(X, test_model_maker, [2, 3, 4, 5, 6], name='silhouette')
    """
    clusterer = KMeans(n_clusters=n_clusters, random_state=10)
    return clusterer


def plot_silhouette(X, model_maker, cluster_list, name=None):
    ...
    for n_clusters in cluster_list:
        ...
        clusterer = model_maker(n_clusters=n_clusters)
        cluster_labels = clusterer.fit_predict(X)
        ...
        if (name == None):
            ...
            plt.savefig("images/" + name + "_" + str(n_clusters) + ".png")
            plt.close(fig)
...
def run_all_09():
    ...
    plot_silhouette(X, test_model_maker, list(range(2, 5)), name='silhouette2')
